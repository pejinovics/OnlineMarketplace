...
//    }
    ...
    private String doGenerateToken(Map<String, Object> claims, String subject, UserDetails userDetails) {
        return Jwts.builder().setClaims(claims).setSubject(subject).claim("id",userService.findByUsername(userDetails.getUsername()).get().getId()).claim("Authorities", userDetails.getAuthorities().stream().map(authority -> authority.getAuthority().replace("ROLE_", "")).collect(Collectors.toSet())).setIssuedAt(new Date(System.currentTimeMillis()))
